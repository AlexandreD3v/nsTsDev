...
/**
 * Module for creating and fetching ACS Bank Accounts in NetSuite.
 *
 ...
 * @exports - Functions for creating and fetching ACS Bank Accounts.
 */
 ...
 */
export function createAcsBankAccountRecord(acc: any, vendorId: number, accRet?: any): number {
    var ret;
    try {
        ...
        if (isNaN(acc.bank)) createAcsBankAccount.setValue(rlEnum.nsColunms.accBancoName, "" + acc.bank);
        // If the bank digit exists, set the bank digit in the record to be saved.
        if (+rlEnum.nsColunms.accDigitoBanco > 0) createAcsBankAccount.setValue(rlEnum.nsColunms.accDigitoBanco, "" + acc.accountDigit);
        // If the vendor digit exists, set the vendor in the record to be saved.
        if (vendorId > 0) createAcsBankAccount.setValue(rlEnum.nsColunms.accFornecedor, "" + vendorId);
 ...
 */
export function fetchAcsBankAccount(objAcsBankAccount: any, vendId: number): object | number {
    var ret;
    try {
 ...
 */
function fetchAcsBankDetId(cod: string): number {
    var ret;
    try {
 ...
 */
export function setBankAccount(accountRet: { id: number, idVendor: number }, idVendor: number): number {
    var ret;
    try {
 ...
 */
function buildFilterDigitoBanco(bankDigit: string): Array<any> | number {
    var ret;
    try {
